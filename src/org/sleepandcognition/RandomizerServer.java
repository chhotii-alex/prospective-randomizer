			...
			while (argNum < args.length) {
				...
				if (currArg.length() == 2 && currArg.charAt(0) == '-') {
					...
					case 'h':
						...
						break;
					case 'v':
						System.out.println(String.format("Balancing Randomizer version %d", Randomizer.RandomizerCommVersion()));
						break;
					case 'b':
						verbosity = 1;
						break;
		...
		if (guiMode) {
			...
			if (networkMode) {
				...
				try {
					...
					System.err.println("This could be because an existing instance of RandomizerServer is running on this machine.");
					System.err.println("On Mac or Linux, use the ps command (probably with some flags) to hunt down previous instances");
					System.err.println("of this process, and use kill -9 to kill them.");
					System.err.println("On Windows, use ctl-alt=del to get a list of processes, and look for and stop javaw.");
